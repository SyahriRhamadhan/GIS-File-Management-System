...
4. Hasil:
   - Jika sukses, sistem membuat sesi baru (regenerate session) lalu redirect ke halaman *intended*.
   - Jika tidak ada *intended*, redirect ke `/dashboard`.
...
- Tambah Kategori: `/dashboard/kategori`
...
Catatan performa:
- Metadata GeoJSON dapat di-*lazy-load* melalui endpoint `GET /api/dashboard/geojsons`.
...
3. Pastikan peta/preview tampil dan polygon ter-*center*.
...
3. (Opsional) Konversi SHP/KML/KMZ → GeoJSON di halaman create:
   - File hasil konversi > **10MB** hanya dapat di-*download* (tidak disarankan dipakai langsung di form).
...
4. Preview:
   - Peta akan men-*zoom* ke bounds data.
